...
namespace UnityEditor.U2D.Aseprite.Common
{
    ...
    internal static class ImagePacker
    {
        ...
        public static void Pack(Vector2Int canvasSize, RectInt[] cellRects, int padding, bool requireSquarePOT, out RectInt[] outPackedRects, out int outPackedWidth, out int outPackedHeight)
        {
            ImagePackNode packNode = InternalPack(canvasSize, cellRects.Length, padding, requireSquarePOT);
            ...
            outPackedRects = new RectInt[cellRects.Length];
            for (int i = 0; i < cellRects.Length; ++i)
                outPackedRects[i] = new RectInt(visitor.positions[i].x + padding, visitor.positions[i].y + padding, canvasSize.x, canvasSize.y);
#if PACKING_DEBUG
            var emptyNodeCollector = new CollectEmptyNodePositionVisitor();
            packNode.AcceptVisitor(emptyNodeCollector);
            Array.Resize(ref outPackedRects, cellRects.Length + emptyNodeCollector.emptyAreas.Count);
            for (int i = cellRects.Length; i < outPackedRects.Length; ++i)
                outPackedRects[i] = emptyNodeCollector.emptyAreas[i - cellRects.Length];
#endif
        }
        ...
        public static void Pack(Vector2Int canvasSize, NativeArray<Color32>[] buffers, RectInt[] cellRects, int padding, uint spriteSizeExpand, bool requireSquarePOT, out NativeArray<Color32> outPackedBuffer, out int outPackedBufferWidth, out int outPackedBufferHeight, out RectInt[] outPackedRect, out Vector2Int[] outUVTransform)
        {
            ...
            try
            {
                RectInt[] tightRects = FindTightRectJob.Execute(buffers, cellRects);
                RectInt[] tightRectArea = new RectInt[tightRects.Length];
                ...
                Pack(canvasSize, cellRects, padding, requireSquarePOT, out outPackedRect, out outPackedBufferWidth, out outPackedBufferHeight);
                ulong packBufferSize = (ulong)outPackedBufferWidth * (ulong)outPackedBufferHeight;
                ...
                Blit(canvasSize, cellRects, outPackedBuffer, outPackedRect, outPackedBufferWidth, buffers, tightRects, cellRects, padding);
            }
            catch (Exception ex)
            {
        ...
        static ImagePackNode InternalPack(Vector2Int canvasSize, int rectCount, int padding, bool requireSquarePOT)
        {
            if (rectCount == 0)
                return new ImagePackNode() { rect = new RectInt(0, 0, 0, 0) };
            var sortedRects = new ImagePackRect[rectCount];
            for (var i = 0; i < rectCount; ++i)
            {
                sortedRects[i] = new ImagePackRect();
                sortedRects[i].rect = new RectInt(0, 0, canvasSize.x, canvasSize.y);
                sortedRects[i].index = i;
            }
            var initialWidth = (int)NextPowerOfTwo((ulong)canvasSize.x);
            var initialHeight = (int)NextPowerOfTwo((ulong)canvasSize.y);
            if (requireSquarePOT)
        ...
        public static void Blit(Vector2Int canvasSize, RectInt[] cellRects, NativeArray<Color32> buffer, RectInt[] blitToArea, int bufferBytesPerRow, NativeArray<Color32>[] originalBuffer, RectInt[] blitFromArea, RectInt[] rects, int padding)
        {
            ...
            {
                int4 fromArea = new int4(blitFromArea[bufferIndex].x, blitFromArea[bufferIndex].y, blitFromArea[bufferIndex].width, blitFromArea[bufferIndex].height);
                int4 toArea = new int4(
                    blitToArea[bufferIndex].x + cellRects[bufferIndex].x,
                    blitToArea[bufferIndex].y + cellRects[bufferIndex].y,
                    cellRects[bufferIndex].width,
                    cellRects[bufferIndex].height
                );
                ...
                unsafe
                {
                    ...
                    BurstedBlit(originalBufferPtr, in fromArea, in toArea, rects[bufferIndex].width, bufferBytesPerRow, outputBufferPtr);
                }
            }
